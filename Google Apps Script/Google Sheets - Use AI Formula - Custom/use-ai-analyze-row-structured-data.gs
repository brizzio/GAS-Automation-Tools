/**
 * AIAnalyzeRow  –  v2  (no web search, structured output only)
 *
 ...
 *        "list" | "json" | "options:value1,value2,value3" | "multioptions:value1,value2,value3"
 *
 * @return {string|number}  Model response, already parsed/cleaned.
 */
function AIAnalyzeRow(headerRange, dataRange, prompt, outputType) {
  ...
  if (!schema) {
    // Simple text response - use regular model
    const textPrompt = `
${prompt}
...
`.trim();
    ...
    const response = callOpenAIRegular(textPrompt, apiKey);
    return processResponse(response, outputType);
  } else {
    // Structured response needed - use schema model
    const structPrompt = `
${prompt}
...
Respond with JSON that matches the required schema exactly.
`.trim();
...
/** Regular OpenAI call for text responses */
function callOpenAIRegular(message, apiKey) {
  const payload = {
    model: 'gpt-4o-mini',
...
/** Structured-output call – for when we need specific formats */
...
/** Short, plain-language hints for the model. */
